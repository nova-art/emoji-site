...
(function () {
  ...
  // --- Pet Mode Toggle ---
  const petModeToggle = document.getElementById('pet-mode-toggle');
  let isPetModeEnabled = localStorage.getItem('emoji_pet_mode') !== 'false';
  
  // Check if the build explicitly disabled pet mode
  const petModeMeta = document.querySelector('meta[name="pet-mode"]');
  if (petModeMeta && petModeMeta.content === 'disabled') {
    isPetModeEnabled = false;
  }
  
  if (petModeToggle) {
    petModeToggle.checked = isPetModeEnabled;
    petModeToggle.addEventListener('change', (e) => {
      isPetModeEnabled = e.target.checked;
      localStorage.setItem('emoji_pet_mode', isPetModeEnabled);
      window.location.reload();
    });
  }

  // --- Emoji Adoption (Dynamic Status) ---
  if (isPetModeEnabled) {
    const EMOJI_API = '/api/emojis/status';
      ...
      if (allCards) {
        allCards.forEach(card => {
          ...
          if (statusMap[char]) {
            ...
            let badge = card.querySelector('.emoji-card__adopted');
            if (!badge) {
              badge = document.createElement('div');
              badge.className = 'emoji-card__adopted';
              card.appendChild(badge);
            }
            badge.innerHTML = `Owned by <strong>${statusMap[char].name}</strong>`;
            
            // Clicking adopted card goes to play instead of copying
            card.href = '/play';
            card.addEventListener('click', (e) => {
               // Prevent copy on adopted cards
               e.stopPropagation();
            });
          } else {
            // If it's a grid item (has href), change it to go to detail
            // Actually the static site already has them as links if it's the index page.
          }
        });
      }

      // Update Detail Page Hero
      const emojiDisplay = document.getElementById('emoji-display');
      if (emojiDisplay) {
        const char = emojiDisplay.textContent;
        const hero = document.querySelector('.emoji-detail__hero');
        
        const actionContainer = document.createElement('div');
        actionContainer.className = 'adopt-action-container';
        
        if (statusMap[char]) {
          actionContainer.innerHTML = `
            <div class="adopted-badge">Adopted by <strong>${statusMap[char].name}</strong></div>
            <a href="/play" class="view-pet-btn">View in Game</a>
          `;
        } else {
          actionContainer.innerHTML = `
            <a href="/play?adopt=${encodeURIComponent(char)}" class="adopt-btn">Adopt this Emoji</a>
          `;
        }
        hero.appendChild(actionContainer);
      }
    })
    .catch(err => console.warn("Could not load adoption status. Playing offline?"));
  }
